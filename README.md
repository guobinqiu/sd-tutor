...
要实现照片中的效果，光有prompt提示词是远远不够的，我们首先要去安装stable diffusion webui，它是stable diffusion（免费，没有生成次数限制）的一个用户操作界面，方便我们文生图，图生图，局部生图等。
...
- https://github.com/AUTOMATIC1111/stable-diffusion-webui webui官方安装教程

- 最好有vpn，没有也没关系，有的文件我已经为你下载好了
...
```
`Files`下面推荐选`Windows installer (64-bit)`
...
Lora为例，小模型可以到 https://huggingface.co/ 上去下载，下载后放到`stable-diffusion-webui/models/Lora`目录下。
...
比如`PiscesArmor.safetensors`这个模型我们在 https://huggingface.co/ 上没有找到，可以去 https://civitai.com/ 搜搜看。打开civitai网站需要vpn，从它上面下载模型则不需要，用chrome浏览器f12定位出下载链接关闭vpn再下载！

小模型比较小，一般只有几百兆，考虑到有些小伙伴没有vpn的，所以我事先下载了几个放目录里了。

### 4.运行webui

运行webui之前，我们先要修改下一下`launch.py`文件，让它从国内镜像下载依赖包
...
Width|图像宽度，取值建议<=权重最高的小模型的宽，规范的小模型会标注所使用的训练图片的尺寸
Height|图像高度，取值建议>=权重最高的小模型的高，规范的小模型会标注所使用的训练图片的尺寸
